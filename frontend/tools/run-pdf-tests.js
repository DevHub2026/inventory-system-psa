// Puppeteer test harness to generate QR sheet PDFs using the shared layout logic
...
async function run() {
  ...
  // Expose a small HTML page where scripts run
  await page.setContent('<!doctype html><html><head><meta charset="utf-8"></head><body></body></html>')

  // Load jsPDF from CDN (UMD) so doc is available as window.jspdf.jsPDF
  await page.addScriptTag({ url: 'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js' })

  // Run test inside page context
  const pdfBase64 = await page.evaluate(async () => {
    // helper mm->px
    function mmToPx(mm, dpi = 600) { return Math.round((mm / 25.4) * dpi) }

    // simplified calculateLayout (copied from app)
    function calculateLayout(settings, assetsList) {
      ...
      let rowsN
      if (rowsSetting === 'auto') {
        rowsN = Math.max(1, Math.floor((paperH - 2*mg + gp) / (lh + gp)))
    ...
    // Simple QR generator: returns an SVG with a placeholder black square sized to qrMm
    ...
    // render SVG to PNG data URL using canvas
    ...
    // generate 28 mock assets with long names for coverage
    const assets = []
    for (let i = 1; i <= 28; i++) {
    ...
    // use jsPDF from UMD (window.jspdf.jsPDF)
    const { jsPDF } = window.jspdf
    const doc = new jsPDF({ unit: 'mm', format: [layout.paperW, layout.paperH], orientation: 'portrait' })
    const DPI = 600

    // simple text fit - naive: use fixed font size 12 and wrap if needed
