...
import { KeyRound, User, Mail, Lock, Eye, EyeOff, Shield, Building2, Hash, Check, X, Bot, AlertTriangle } from 'lucide-react'
              ...
              >
                  ...
                  <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                    {isHeavy ? (
                      <span style={{ color: T.amberText, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <AlertTriangle size={14} /> ⚠️ Heavier model. May significantly increase CPU usage, RAM consumption, and response latency on this 16 GB CPU-only server.
                      </span>
                    ) : (
                ...
                <div style={{ paddingLeft: 16 }}>
                  {isSaving && !isActive ? (
                    ...
                    <div style={{ width: 24, height: 24, borderRadius: '50%', border: `2px solid ${T.border}` }} />
                  )}
                </div>
              </div>
            );
...
export function SettingsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 820, paddingBottom: 32 }}>
      ...
      {/* AI Settings Section - Only for Admins */}
      {user?.permissions?.includes('system.settings.update') && (
        <AISettingsSection />
      )}
    </div>
  )
