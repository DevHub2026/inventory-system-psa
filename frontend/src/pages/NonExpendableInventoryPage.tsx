...
export function NonExpendableInventoryPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Back */}
      <button type="button" onClick={() => navigate('/inventory')} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, color: '#0B3D91', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <Input label="Item Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Desktop Computer" />
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Input label="Reason" value={stockReason} onChange={(e) => setStockReason(e.target.value)} placeholder={stockType === 'in' ? 'New assets received' : 'Disposed / transferred'} />
        </div>
      </Modal>
      ...
      >
        {adjustItem && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <Input label="Reason" value={adjustReason} onChange={(e) => setAdjustReason(e.target.value)} placeholder="Physical count correction, transfer, disposal..." />
          </div>
        )}
      </Modal>
